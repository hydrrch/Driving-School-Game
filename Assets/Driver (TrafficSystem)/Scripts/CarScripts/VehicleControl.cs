...
public class VehicleControl : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        if (AIVScript.vehicleStatus != VehicleStatus.EmptyOff)
        {
            ...
            foreach (Light brakeLight in carLights.brakeLights)
            {
                ...
                {
                    brakeLight.intensity = Mathf.MoveTowards(brakeLight.intensity, carLights.brakeLightIntensity, 0.5f);
                }
                else
                {
            ...
            foreach (Light WLight in carLights.reverseLights)
            {
                ...
                {
                    WLight.intensity = Mathf.MoveTowards(WLight.intensity, carLights.reverseLightIntensity, 0.5f);
                }
                else
                {
    ...
    : slip = Mathf.Lerp(slip, 0.01f, 0.02f);
            ...
            if (shift && speed > 30.0f)
            {
                carSounds.nitro.volume = Mathf.MoveTowards(carSounds.nitro.volume, 1.0f, Time.deltaTime * 2.0f);
            }
            else
            {
                carSounds.nitro.volume = Mathf.MoveTowards(carSounds.nitro.volume, 0.0f, Time.deltaTime * 2.0f);
        ...
        if (AIVScript.vehicleStatus != VehicleStatus.EmptyOff)
        {
            ...
            if (carSounds.IdleEngine != null)
            {
                ...
                if (Mathf.Abs(accel) > 0.3f)
                    accel = 0.3f;
                carSounds.IdleEngine.volume = Mathf.MoveTowards(carSounds.IdleEngine.volume, 0.2f + Mathf.Abs(accel), 0.01f);
