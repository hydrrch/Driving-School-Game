...
public class AIVehicle : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        //Car Reset
        if (this.vehicleStatus.Equals(VehicleStatus.Player))
        {
            ResetCar();
        }
        ...
        for (int i = 0; i < 6; i++)
        {
            ...
            if (Physics.Raycast(raycastPoint.TransformPoint((Mathf.Repeat((float)i, 3) - 1), 0, 0), raycastPoint.TransformDirection((rotate * rayCastAngle), 0, (lentgh * rayCastLentgh)), out raycastHit, (lentgh * rayCastLentgh), layerMask.value))
            {
                ...
                //else //stop for traffic vehicle 
                //{
                //    if (raycastHit.transform.root.GetComponent<AIVehicle>())
                //    {
                //        AIBrake = true;
                //    }
                //}
        ...
        if (Vector3.Distance(raycastPoint.position, player.position) < 10)
            AIBrake = true;
