...
public class CreateAI : MonoBehaviour
{
    ...
    [SerializeField] float CreateVehicleDist;
    [SerializeField] GameObject currentNode;
    [SerializeField] Collider[] nodes;
    void Update()
    {
        ...
        if (createVehicles)
        {
            if (vehicleTimer == 0)
            {
                //nodes = Physics.OverlapSphere(transform.position, 300, nodeMask);
                nodes = Physics.OverlapSphere(transform.position, CreateVehicleDist, nodeMask);


                foreach (Collider node in nodes)
                {
                    Dist = Vector3.Distance(transform.position, node.transform.position);
