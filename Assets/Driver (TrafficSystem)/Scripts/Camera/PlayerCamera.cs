...
public class PlayerCamera : MonoBehaviour
{
    ...
    public float xSpeedUnity = 150.0f;
    public float ySpeedUnity = 150.0f;
    ...
    public static bool IsLerp = false;
    public static Transform lerpRef;
    [SerializeField] Camera360LookScript CameraLookScriptObj;
    ...
    void LateUpdate()
    {
        ...
        if (!IsLerp)
        {
            ...
            {
                x += Input.GetAxis("Mouse X") * xSpeedUnity * 0.02f;
                y -= Input.GetAxis("Mouse Y") * ySpeedUnity * 0.02f;
            ...
            else if (GameControl.manager.controlMode == ControlMode.touch)
            {
                ...
                x += CameraLookScriptObj.HorizontalVal * xSpeed * /*0.02f*/Time.deltaTime;
                //y -= CnInputManager.GetAxis("Camera Y") * ySpeed * 0.02f;
                y -= CameraLookScriptObj.VerticalVal * ySpeed * /*0.02f*/Time.deltaTime;
        ...
        {
            transform.position = Vector3.Lerp(transform.position, lerpRef.position, LerpSpeed * Time.deltaTime);
            transform.rotation = Quaternion.Lerp(transform.rotation, lerpRef.rotation, LerpSpeed * Time.deltaTime);
        }
