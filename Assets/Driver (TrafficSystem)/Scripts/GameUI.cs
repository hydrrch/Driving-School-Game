...
public class GameUI : MonoBehaviour
{
    ...
    IEnumerator ShowControlsRoutine()
    {
        if (AIContoller.manager.playerCamera.enabled)
            yield return new WaitUntil(() => AIContoller.manager.vehicleCamera.enabled);
        else
            yield return new WaitUntil(() => AIContoller.manager.playerCamera.enabled);
