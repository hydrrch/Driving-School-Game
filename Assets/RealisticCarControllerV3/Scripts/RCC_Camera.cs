...
public class RCC_Camera : MonoBehaviour
{
    ...
    Scrollbar gear;
    public float TPSCamRotationVal;
    ...
    public void ChangeCamera()
    {
        ...
        switch (cameraSwitchCount)
        {
            ...
            case 0:
                ...
                //FindObjectOfType<CamRotatorScript>().Activate_Rotator_Obj(false);
                //StartCoroutine(FindObjectOfType<GUImanager>().Show_Cam_Change_Popup("You Have Selected Third Person Cam View"));
                break;

            case 1:
                if (PlayerPrefs.GetInt("HoodCamera") == 1 || PlayerPrefs.GetInt("HoodCameraTemp") == 1)
                {
                    if (PlayerPrefs.GetInt("HoodCameraSelected") == 1)
                    {
            ...
            case 2:
                if (PlayerPrefs.GetInt("WheelCamera") == 1 || PlayerPrefs.GetInt("WheelCameraTemp") == 1)
                {
                    if (PlayerPrefs.GetInt("WheelCameraSelected") == 1 && PlayerPrefs.GetInt("RightCamSelect") == 1)
                    {
            ...
            case 3:
                if (PlayerPrefs.GetInt("WheelCamera") == 1 || PlayerPrefs.GetInt("WheelCameraTemp") == 1)
                {
                    ...
                    if (PlayerPrefs.GetInt("WheelCameraSelected") == 1 && PlayerPrefs.GetInt("LeftCamSelect") == 1)
                    {
                        if (useWheelCameraMode && wheelCam)
                        {
                            cameraMode = CameraMode.WHEEL;
            ...
            case 6:
                if (PlayerPrefs.GetInt("OrbitCamera") == 1 || PlayerPrefs.GetInt("OrbitCameraTemp") == 1)
                {
                    if (PlayerPrefs.GetInt("OrbitCameraSelected") == 1)
                    {
    ...
    void TPS()
    {
        ...
        #region Back + Orbital View

        //if (!CamRotatorScript.orbital_cam)
        //{
        ...
        //    TPSRotationDamping = 1;

        //    // If speed is lower than -10, rotate the camera to backwards.
        //    //if (speed < -10)
        //    if (gear != null && gear.value > 0.5f)
        //    {
        //        //wantedRotationAngle = playerCar.eulerAngles.y + FindObjectOfType<LevelChanger>().camRotval;
        //        wantedRotationAngle = playerCar.eulerAngles.y + 180;
        //        TPSRotationDamping = 1;
        //    }

        //    // Damp the rotation around the y-axis
        //    currentRotationAngle = Mathf.LerpAngle(currentRotationAngle, wantedRotationAngle, TPSRotationDamping * Time.deltaTime * 5);
        ...
        //    //float wantedRotationAngle = playerCar.eulerAngles.y + TPSCamRotationVal;
        //    //float wantedHeight = playerCar.position.y + TPSHeight;
        //    //float currentRotationAngle = transform.eulerAngles.y; //FindObjectOfType<GUImanager>().X_rot_val
        //    //float currentHeight = transform.position.y;
        ...
        //    wantedRotationAngle = playerCar.eulerAngles.y + TPSCamRotationVal;
        //    float wantedHeight = playerCar.position.y + TPSHeight;
        //    float currentRotationAngle = transform.eulerAngles.y;
        //    float currentHeight = transform.position.y;
        ...
        //    else
        //    {
        //        TPSRotationDamping = Mathf.Lerp(0f, 5f, 10 / 40f);
        //    }
        ...
        if (isFollow)
        {
            ...
            if (gear != null)
            {
                ...
                {
                    //wantedRotationAngle = playerCar.eulerAngles.y + FindObjectOfType<LevelChanger>().camRotval;
                    wantedRotationAngle = playerCar.eulerAngles.y + TPSCamRotationVal + 180;
                    TPSRotationDamping = 1;
                }
            }
    ...
    //    x_rot_val += pointerData.delta.x * 0.1f * 2f;
    //}
    ...
    public void OnDrag(BaseEventData data)
    {
        ...
        //orbitRotation = Quaternion.Euler(orbitY, orbitX, 0);
        //orbitPosition = orbitRotation * new Vector3(0f, 0f, -TPSDistance) + playerCar.position;
        TPSCamRotationVal += pointerData.delta.x * 0.1f * 02f;
    ...
    async void GetAndSetGear()
    {
        ...
        if (!SceneManager.GetActiveScene().name.Equals("Free Mode New Env") && !SceneManager.GetActiveScene().name.Equals("Menu"))
            gear = GameManager.instance.gear;
    }
