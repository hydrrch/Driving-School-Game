...
//				rigid.AddRelativeTorque (Vector3.up * (((steerInput * _gasInput) * 10000f)), ForceMode.Force); 
	...
	internal IEnumerator ChangingGear(int gear){
		...
		if(gearShiftingClips.Length > 0){
			gearShiftingSound = RCC_CreateAudioSource.NewAudioSource(gameObject, "Gear Shifting AudioSource", 0f, 0f, RCCSettings.maxGearShiftingSoundVolume, gearShiftingClips[UnityEngine.Random.Range(0, gearShiftingClips.Length)], false, true, true);
            //if(!gearShiftingSound.isPlaying)
            //	gearShiftingSound.Play();
            gearShiftingSound.Play();
            //print("playing gear sound");
        }
	...
	public void ResetGari()				
	{
		//+++//if (speed < 5.75f && !rigid.isKinematic)
		//{
		...
		//transform.position = GUImanager.resetPoser.position;
		//transform.rotation = GUImanager.resetPoser.rotation;
		resetTime = 0f;
			///GameAnalytics
			PlayerPrefs.SetInt("ResetGari", PlayerPrefs.GetInt("ResetGari") + 1);
			//GameAnalytics.NewDesignEvent("Event_ResetBot", PlayerPrefs.GetInt("ResetBot"));
