    ...
    void Update()
    {
        ...
        switch (lightType)
        {
            ...
            case LightType.BrakeLight:
                Lighting((!carController.lowBeamHeadLightsOn ? (carController._brakeInput >= .1f ? intensity : 0f) : (carController._brakeInput >= .1f ? 0.5f : .3f)));
                break;

            case LightType.ReverseLight:
                Lighting(carController.direction == -1 ? intensity : 0f);
                break;
