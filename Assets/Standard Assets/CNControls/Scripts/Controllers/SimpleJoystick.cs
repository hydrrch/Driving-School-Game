...
namespace CnControls
{
    ...
    public class SimpleJoystick : MonoBehaviour, IDragHandler, IPointerUpHandler, IPointerDownHandler
    {
        ...
        public GameObject JoytickTutorialObject;
        ...
        public void OnPointerDown(PointerEventData eventData)
        {
            ...
            if (JoytickTutorialObject.activeInHierarchy)
                JoytickTutorialObject.SetActive(false);

            if (WelcomeNotes[0].activeInHierarchy)
            {
                for (int i = 0; i < WelcomeNotes.Length; i++)
                {
                    WelcomeNotes[i].SetActive(false);
                }
            }
        }
        [SerializeField] GameObject[] WelcomeNotes;
